...
import IncomeExpenseChart from "../components/features/charts/IncomeExpenseChart";
import SpendingCategoryChart from "../components/features/charts/SpendingCategoryChart";
import MonthlySpendingChart from "../components/features/charts/MonthlySpendingChart";
...
export default function AnalyticsPage() {
  ...
  const summaryData = useMemo(() => {
    ...
    const categoryBreakdown = Object.entries(catSpending).map(
      ([catId, spent]) => {
        const cat = categories.find((c) => c.id === catId);
        const budget = budgets.find(
          (b) => b.categoryId === catId && b.month === summaryMonth,
        );
        return {
          categoryId: catId,
  ...
  const comparisonData = useMemo(() => {
    ...
    const expenseDiffPct =
      expensesB > 0
        ? Math.round(((expensesA - expensesB) / expensesB) * 100)
        : 0;
    return {
      ...
      incomeA,
      expensesA,
      savingsA: incomeA - expensesA,
      incomeB,
      expensesB,
      savingsB: incomeB - expensesB,
      expenseDiffPct,
    };
  ...
  if (transactions.length === 0) {
    return (
      <EmptyState
        icon={
          <i className="fa-solid fa-chart-line text-accent-primary text-2xl" />
        }
  ...
  const barColor = (pct) =>
    pct >= 100 ? "bg-expense" : pct >= 85 ? "bg-yellow-500" : "bg-income";
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                },
                {
                  ...
                  color:
                    summaryData.balance >= 0 ? "text-income" : "text-expense",
                  bg: "bg-bg-tertiary/60 border-border",
                },
              ].map(({ label, value, color, bg }) => (
                <div
                  key={label}
                  className={`glass-panel rounded-xl p-4 border text-center ${bg}`}
                >
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="h-1 bg-bg-tertiary rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(item.budgetUsage, 100)}%`,
                              }}
                            />
                          </div>
                          <p className="text-xs text-text-muted">
                            Budget: {formatCurrency(item.budgetLimit)} —{" "}
                            {item.budgetUsage}% used
                          </p>
                        </div>
                      )}
                    </div>
        ...
        {comparisonData && (
          <div className="glass-panel rounded-xl overflow-hidden">
            ...
            {[
              {
                label: "Income",
                a: comparisonData.incomeA,
                b: comparisonData.incomeB,
                color: "text-income",
              },
              {
                label: "Expenses",
                a: comparisonData.expensesA,
                b: comparisonData.expensesB,
                color: "text-expense",
              },
              {
                label: "Net Savings",
                a: comparisonData.savingsA,
                b: comparisonData.savingsB,
                color: null,
              },
            ].map(({ label, a, b, color }) => (
              <div
                key={label}
                className="grid grid-cols-3 px-5 py-3.5 border-b border-border hover:bg-bg-hover transition-colors"
              >
                <span className="text-sm text-text-muted">{label}</span>
                <span
                  className={`text-sm font-semibold text-center ${color ?? (a >= 0 ? "text-income" : "text-expense")}`}
                >
                  {formatCurrency(a)}
                </span>
                <span
                  className={`text-sm font-semibold text-center ${color ?? (b >= 0 ? "text-income" : "text-expense")}`}
                >
                  {formatCurrency(b)}
                </span>
              </div>
                ...
                {comparisonData.expensesB > 0 ? (
                  <>
                    <strong className="text-text-primary">
                      {comparisonData.monthALabel}
                    </strong>{" "}
                    expenses are{" "}
                    <strong
                      className={
                        comparisonData.expenseDiffPct >= 0
                          ? "text-expense"
                          : "text-income"
                      }
                    >
                    ...
                    than{" "}
                    <strong className="text-text-primary">
                      {comparisonData.monthBLabel}
                    </strong>
                    .
                  </>
                ) : (
                  <>
                    Add expense records in{" "}
                    <strong className="text-text-primary">
                      {comparisonData.monthBLabel}
                    </strong>{" "}
                    to compare spending.
