...
export default function TransactionTable({
  ...
  const getCat = (catVal) => {
    if (!catVal || !Array.isArray(categories)) {
      return null;
    }
    ...
    return categories.find((c) => c.id === catVal || c.documentId === catVal);
  };
      ...
      <div className="hidden md:block glass-panel rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {transactions.map((tx) => {
              const cat = getCat(tx.category || tx.categoryId);

              return (
                <tr
                  key={tx.documentId || tx.id}
                  className="hover:bg-bg-hover transition-colors duration-100"
                >
